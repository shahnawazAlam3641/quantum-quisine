...
const Nav = () => (
     <div className="navbar">
        ...
        <div className="logo">
            <img className="nav-img" src = "https://myday24.gumlet.io/iconikai/652d2a861f83e3fb4ece90cf/652d2b1d1f83e3fb4ece9119-aiicon.png?w=200&h=200"/>
            <h1 className="name">QuantumQuisine</h1>
        </div>
...
const FoodCard = (props)=> {
    ...
    <div className="food-card">
        <img className="card-img" src= {"https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/" + restaurantList[0].data?.cloudinaryImageId } />
        <h1 className="card-title">{restaurantList[0].data?.name}</h1>
        <p>
          <span>{restaurantList[0].data?.avgRating } ⋅</span><span> {restaurantList[0].data?.deliveryTime + "mins"}</span>
        </p>
        <p>{restaurantList[0].data?.cuisines.join(", ")}</p>
        <p>{restaurantList[0].data?.locality}</p>
    </div>
    )
}
...
const Body = ()=> {
    return (<div className="body">
        <div className="search-container">
            <input placeholder="Search Your Food" type="text"></input>
            <button type="button" className="btn">Search</button>
        </div>
        <div className="food-container">
            {
              restaurantList.map((restaurant, index) => (
                <FoodCard key={restaurantList[index].data?.id} restaurant = {restaurantList[index]}  />
      ))
            }
        </div>
    </div>)
